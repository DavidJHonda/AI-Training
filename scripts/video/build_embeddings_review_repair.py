...
def main():
 ...
 keep(0,11.166667,'Token refresher')
 keep(14.4,49.6,'Question, student ID and descriptive numbers')
 ...
 event(8,'intro','verified-token-ids')
 event(18.7,'student','student-id-illustration')
 ...
 event(234.766667,'pieces','word-piece-embeddings')
 event(240.06,'pieces','un-row',mark([80,318,1520,442],EP))
 event(240.84,'pieces','belie-row',mark([80,488,1520,612],BLUE))
 event(241.56,'pieces','vable-row',mark([80,658,1520,782],TEAL))
 event(242.54,'pieces','all-pieces-have-rows')
 def source_at(f):
 ...
 m=dict(source=str(SOURCE),donor=str(DONOR),output=str(DEST),fps=FPS,total_frames=total,duration=total/FPS,close_start_frame=close_start,timeline=timeline,states=schedule,outline_checks=checks,protected_hashes=hashes,assets={k:dict(path=str(p),sha256=sha(p)) for k,p in assets.items()},boundaries=[dict(frame=f,label=l) for f,l in sorted(boundaries.items())],audio=dict(room_tone_source=[110.8,110.97],donor_gain=gain,donor_span=[189.633333,195.566667],sample_rate=SR),close_camera=dict(prehold_frames=48,push_frames=150,zoom_endpoint=1.2,settled_frames=total-close_start-198))
 ...
 for f in range(total):
  if f<close_start:
   while f>=schedule[idx]['end_frame']:idx+=1
   e=schedule[idx];im=cache[native_needed[f]] if e['board']=='native' else states[e['label']]
  else:
