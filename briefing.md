...
## Lesson map
69 lessons across 7 section groups (a five-part content spine plus Start Smarter and Finish), listed in delivery order. Each lesson is listed as `Displayed Label (internal id)`. The id is the React component key and navigation reference used in code; the label is what students see in the UI.
...
- Understand AI (20): one section group spanning three movements (former Foundations, Inside AI, AI Answers; internal order unchanged):
  - Foundations movement: Opener (openerfoundations), AI is Math (aiismath), How We Got Here (howwegothere), Rules vs Patterns (aivscode), No One Wrote the Rules (norules), AI's Fuel (data)
...
Structure notes (after the May 2026 restructure): the spine is Start Smarter → Understand AI → Work With AI → Check the Output → Protect Yourself → Build Your Advantage → Finish. Part badges read "NN / 7" and are derived from `SECTION_GROUPS` via `partBadge()`, not hand-typed; Start Smarter and Finish carry no badge, and within Understand AI only openerfoundations carries the part badge while openerinside/openeranswers show a quiet "Understand AI · N of 3" movement label. The four former section openers (openerusing, openercontrols, openertraps, openerjudgment) are retired: still defined in the file as dead components, but removed from `SECTION_GROUPS` and `SECTION_COMPONENTS`. The Build Your Advantage opener keeps the component id `openerrealworld` (displayed title "Build Your Advantage"). Studying/Integrity/Privacy now live in Protect Yourself (the responsible-use home), no longer "parked."
