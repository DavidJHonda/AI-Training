...
Stroke: the shipped video was built under the old constant-5-px ring rule. The artwork-scaled rule
(owner, 2026-09-21) would draw 3 px here, because this tall board is letterboxed at full view - a
visible change to highlights David asked to keep as they are. The spec therefore pins
`ring_stroke_px: 5`, so the artwork is the only thing that changes on screen.
...
DEST = ROOT / 'Prompts/does-ai-think-v6.mp4'
...
def main() -> None:
    ...
    # 2. The shipped leg spec, verbatim, on the new canvas: same beats, same rings, pinned stroke.
    shipped = json.loads(SHIPPED_LEG.read_text())
    spec = {**shipped, 'image': str(canvas_path), 'ring_stroke_px': 5}
    ...
    manifest = {'candidate': str(DEST), 'candidate_sha256': sha(DEST), 'baseline': str(BASELINE), 'baseline_sha256': BASELINE_SHA,
                ...
                'shipped_leg_spec': str(SHIPPED_LEG), 'rings': spec['rings'], 'ring_stroke_px': 5,
                'replaced_output_frames': list(SPAN), 'replaced_seconds': [SPAN[0] / FPS, SPAN[1] / FPS],
                'treatment': 'compact, still, full view, five row rings and the banner (as shipped)',
