...
OUT = ROOT / "video-audit/training-bias-comparison-2026-09-21/build-v4"
DEST = ROOT / "Prompts/training-bias-v4.mp4"
...
def main():
    ...
    cards = [(34, 536), (550, 1054), (1068, 1570)]
    card = lambda i, top, bot: [cards[i][0], top, cards[i][1], bot]
    b.board("skew", SKEW, SKEW_IN, SKEW_OUT, "compact", [
        target("Defaults", 62.02, card(0, 125, 650), PURPLE),
        target("Blind Spots", 71.22, card(1, 125, 650), BLUE),
        target("Wrong Patterns", 78.20, card(2, 125, 650), AMBER),
    ], push=False)
    b.board("questions", QUESTIONS, Q_IN, Q_OUT, "compact", [
        target("Ask What's Missing", 120.70, card(0, 125, 650), PURPLE),
        target("Ask for Exceptions", 125.04, card(1, 125, 650), BLUE),
        target("Remove the Famous", 132.42, card(2, 125, 650), AMBER),
    ], banner_at=139.48, banner=[40, 691, 1560, 779], push=False)
    b.board("rag", RAG, RAG_IN, RAG_OUT, "compact", [
        target("Retrieve", 210.64, card(0, 125, 609), PURPLE),
        target("Add to Context", 216.98, card(1, 125, 609), BLUE),
        target("Generate", 224.92, card(2, 125, 609), TEAL),
    ], push=False)
