#!/usr/bin/env python3
"""Build Context Window v4 from context-window-2 (2026-09-21 reroll review: rolls 1 and 2 vs. the live v3).
...
CMP_IN, CMP_OUT, CMP_LEG_OUT = 330, 2310, 2585   # 0:11.00 (quiet 10.68-11.20); out 1:17.00 (quiet 76.69-77.35).
                                                 # The leg runs long so the audio-only graft below can hold the
                                                 # board without replaying frames the timeline uses again.
...
GA_PICTURE, CMP_RESUME = 1877, 2152
...
CMP_L, CMP_R = [40, 279, 792, 1107], [816, 279, 1568, 1107]
HS_COLS = ([38, 127, 531, 1065], [554, 127, 1049, 1065], [1072, 127, 1567, 1065])
HS_BANNER = [40, 1093, 1560, 1181]
OUT_CARDS = ([40, 126, 793, 772], [815, 126, 1570, 772], [40, 806, 793, 1405], [815, 806, 1570, 1405])
# the bottom row's white box runs to 1405, not 1286: measuring it at x=1200 stopped at a line gap and
# left both lower rings cutting through their own body text (caught in the v1 state sheet)
OUT_BANNER = [41, 1416, 1559, 1504]

def target(label, at, rect, color, radius=20):
...
def main():
    ...
    b.graft(WHY_DONOR, GA_IN, GA_OUT, "Why the answers differ: Nate said he loves pickup trucks (roll 1)",
            "why", picture_from=GA_PICTURE, visual="compare")
    b.keep(GA_AT, CMP_OUT, "Same Question. Different Answers.", "compare", video_from=CMP_RESUME)
    ...
    b.board("compare", COMPARE, CMP_IN, CMP_LEG_OUT, "compact", [
    ...
    # Board 3 (1600x1221, three columns): compact, one ring per column as its feature is taught.
    b.board("headstart", HEADSTART, HS_IN, HS_OUT, "compact", [
        target("Personalization", 112.75, list(HS_COLS[0]), PURPLE),
        target("Saved Memory", 128.16, list(HS_COLS[1]), BLUE),
        target("Projects", 143.65, list(HS_COLS[2]), TEAL),
    ], banner_at=158.79, banner=HS_BANNER, push=False)

    # Board 4 (1600x1545, four cards in two rows): compact, one ring per card as each is named.
    b.board("outside", OUTSIDE, OUT_IN, OUT_OUT, "compact", [
        target("Older Chats", 170.64, list(OUT_CARDS[0]), PURPLE),
        target("Web Pages", 179.82, list(OUT_CARDS[1]), BLUE),
        target("Files on Your Computer", 188.60, list(OUT_CARDS[2]), TEAL),
        target("Other Apps and Tabs", 196.90, list(OUT_CARDS[3]), AMBER),
    ], banner_at=206.12, banner=OUT_BANNER, push=False)
