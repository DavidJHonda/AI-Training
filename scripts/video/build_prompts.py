...
def build(slug, title, lo, hi, boards, body, numbers, props, required, extra=(), duration=None):
    ...
    timing = (duration if duration is not None else
              f"between {lo} and {hi} minutes long — do not come in under {lo}")
    head = f'Create a video overview of the attached lesson "{title}", {timing}. {src}\n\n'
...
Then the hinge, and say it as written because the whole argument turns on it: AI makes answers cheap to get, that does not make humans less valuable, it changes where value lives. Then board 2, held while both halves are walked: Pre-AI, finding the answer, when knowing where to look and how to uncover a reliable answer was the valuable skill; against With AI, asking the right question, where the valuable skill is deciding what to ask, providing the right context, and judging whether the answer helps. Do not add a redundant footer beneath the comparison.
...
add("welcome", title="Welcome", lo=2.25, hi="2.75", boards=True,
...
On board 4, walk all three cards: laptop or desktop for hands-on activities; ChatGPT for Teens as the main lab tool, with free accounts placing ages 13 to 17 in the teen experience; and a free Google account used to create a Gemini Notebook. Say nothing to install, no paid plan required, "Gemini Notebook," never "NotebookLM," and never Claude.
