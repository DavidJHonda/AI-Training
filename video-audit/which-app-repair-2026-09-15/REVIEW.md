# Which App? (modelselection) v1: review candidate (2026-09-15, EDIT-SPEC best-of build from roll 1 + roll 2)

**Candidate:** `videos/which-app-v1.mp4` (3:49.5, 6885 frames, 30 fps). Roll 1 is the base; roll 2 donates the Gemini card and Board 3's
...
**Live video and lesson unchanged** (`videos/which-app.mp4` hash-verified; `lessons/which-app.md` is mid-edit for the attribution boards and
...
"Claude" mentions).
**At ship:** move to `videos/which-app.mp4`, new cache key on the `modelselection` entry, duration pill 4 min (3:50). Live video unchanged until then.
