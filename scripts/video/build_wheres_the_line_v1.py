#!/usr/bin/env python3
"""Where's the Line? v1 review candidate from wheres-the-line-9-21-1 (2026-09-21 roll). Review only.
...
2026-09-23). Roll 9-21-1 is the narration (7 of 7 required lines exact, every beat present). Changes:
  ...
  pictures: canonical Board 1 from the roll's cut to its INVESTIGATING collage (94.00) to its cut to the crown (137.83), compact,
      ...
      close hold faded to silence (Transformer v10 pattern). Live assets, all six raw rolls, lesson, boards, index.html unchanged.
"""
...
OUT = ROOT / "video-audit/wheres-the-line-comparison-2026-09-23/build-v1"
DEST = P / "wheres-the-line-v1.mp4"
...
def main():
    ...
    w_last2 = donor_wav(b, LAST2, "ethics"); w_9212 = donor_wav(b, R9212, "own")
    ...
    # cut c (4613 -> 4806): "This board outlines four actionable moves ... when deploying AI." removed
    b.pause(18, "breath before the four moves (natural ~0.4 s across the cut -> ~1.0 s)")
    ...
    b.manifest({
        ...
        "narration_changes": {
            ...
            "splice_gaps_frames": {"after_cut_a": 6, "after_cut_b": 9, "before_moves": 18, "before_close": 15}, "close_audio_end": CLOSE_END},
        ...
        "longest_board_runs_seconds": {"uses": round((B1_OUT - B1_IN - (A_IN - A_OUT) - (B_IN - B_OUT) + 15) / FPS, 1), "moves": round((D_OUT + LEAD - B2_IN - (C_IN - C_OUT) + 18) / FPS, 1)},
        "rewind_symlink": str(REWIND),
    })
    print("Prepared", b.total, f"frames ({b.total / FPS:.2f}s)", "gains ethics/struggle/own", g_ethics, g_strug, g_own, "dB", "LEAD", LEAD, flush=True)
