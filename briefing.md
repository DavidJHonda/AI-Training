...
## Course structure
8 section groups, 54 lessons total. Each lesson is listed as `Displayed Label (internal id)`. The id is the React component key and navigation reference used in code; the label is what students see in the UI.

- Intro (4): Pre-Check (preassessment), Why Learn AI? (whydeeper), The Human Part of the Job (humanjob), What You'll Build (intro)
- Foundations (6): What Is AI? (aihistory), How We Got Here (howwegothere), Rules vs Patterns (aivscode), No One Wrote the Rules (blackbox), Generative (generative), AI's Fuel (data)
- Building the Model (6): Training (training), Training Bias (trainingbias), Tokens (tokens), Meaning (embeddings), Behind the Numbers (behindthenumbers), Transformer Architecture (howreads)
- Producing an Answer (7): Context Window (context), Probability (probability), Prediction (prediction), Layers (layers), Attention (attention), Update the Meaning (transform), From Prompt to Output (inference)
...
- Using AI Well (12): The AI Trust Test (aistrengths), Hallucination (hallucination), Critical Thinking & AI (critical), The Mind Trap (mindtrap), The Flattery Trap (flattery), The Engagement Trap (engagementtrap), The Art of Prompting (prompting), Your Thought Partner (thoughtpartner), Studying With AI (studying), How to Verify (verify), Evaluating the Results (evaluating), When AI Acts (whenaiacts)
- AI in the Real World (8): Questions Matter (questionsvaluable), Skills That Matter (humanedge), Academic Integrity (integrity), Privacy & Awareness (privacy), When AI Judges You (judged), Seeing Isn't Proof (synthetic), Work Changes (workchanges), AI & The Future (aifuture)
...
- Planned: section opener pages. Each section group except Intro and Finish Line will get an Opener page at position 1. Purpose: engagement bridge that names why the upcoming lessons cohere and what students will get from the section. Shorter than typical lessons, no TRY IT, completion gate kept light or omitted. ID convention: `opener` prefix + lowercase group name. The six planned ids are openerfoundations, openerbuilding, openerproducing, openercontrols, openerusing, openerrealworld. Content not yet drafted. Placeholder pages are explicitly not being added to index.html until each opener has real content, since validators require every SECTIONS entry to have a SECTION_COMPONENTS mapping and stub pages would visibly degrade the app.
