...
- Understand AI (16): one section group with a single opener; the former Inside AI and AI Answers sub-openers were removed and their framing folded into tokens (top) and training (end):
  - Foundations: Opener (openerfoundations), AI is Math (aiismath), How We Got Here (howwegothere), Rules vs Patterns (aivscode), No One Wrote the Rules (norules), AI's Fuel (data)
  ...
  - How AI builds an answer: Your Input & the Context Window (prompt), Probability (probability), Inference: From Prompt to Output (inference). The former Patterns lesson (patterns) was removed 2026-06-03 as redundant: its one unique idea (training writes patterns into the weights, inference reads them back) folded into the Probability intro, and the term "weights" now debuts in Training. Cut content is logged in docs/parking-lot.md.
