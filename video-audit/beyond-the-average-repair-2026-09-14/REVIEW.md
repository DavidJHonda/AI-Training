# Beyond the Average (whybother; formerly Does School Matter?) v2: review candidate (2026-09-14 PM, EDIT-SPEC build)

**Candidate:** `videos/beyond-the-average-v2.mp4` (3:03.1, 5493 frames, 30 fps). v2 answers David's two notes on v1: (1) 0:55–1:16 was
...
and dense boards open static (no push on a full board). v1 superseded. Named for the lesson's new title (David retitled the
...
| --- | --- | --- | --- | --- |
| Same Tool. Different Advantage. (`illustrations/does-school-matter-same-tool-v1.jpg`, faces; not uploaded) | 1659–2451 | after the pause at 53.3, with roll 2's "The difference is what you add…" | full board, still, banner visible; then roll 1 resumes at "In a future where everyone has the same software…" (72.82) through "…develop those assets." | the first cut (89.55) |
...
Run on the 21:09 render (v2).

1. Decoded frames 5493 = plan; audio 183.104 s (one AAC frame over plan). Each leg decoded its span exactly.
2. `transition_guard.py` passed all 6 declared boundaries (1659, 1947, 2481, 3258, 3537, 5148); `boundary-pairs.jpg` inspected.
