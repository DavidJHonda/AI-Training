...
2. Upload the lesson Markdown and only the JPGs identified as Notebook sources. Do **not** upload `Master Prompt.md`, production instructions, manifests, or older lesson PDFs. Visible-face boards remain in the course and are inserted after generation; reserve their narration and keep their numbered slots rather than renumbering.
3. Paste the per-lesson prompt into the video customization box. Keep it under **500 words** and include the necessary production rules there. Most use `<slug>-video-prompt.txt`; honor a kit's explicitly listed filename when it differs.
