...
from build_embeddings_v4 import student_frame
p=Path('video-audit/embeddings-repair-2026-09-10');m=json.loads((p/'edit-manifest.json').read_text());ff=imageio_ffmpeg.get_ffmpeg_exe()
...
for t in [18.7,22.733333,23.4,24.133333,26.3,29.266667,29.7,30.166667,38]:targets[round(t*30)]='camera-'+str(round(t*30))
while True:
 ok,img=cap.read()
 if not ok:break
 ...
 if i<m['close_start_frame']:
  ...
  st=m['states'][idx]
  if st['board']!='native':
   expected=student_frame(cv2.imread(m['assets']['student']['path']),i) if st['board']=='student' else states[st['label']]
   diff=np.abs(img.astype(float)-expected.astype(float)).mean();worst=max(worst,diff);assert diff<3,(i,st['label'],diff);boardframes+=1
 i+=1
...
cmd=['.video-venv/bin/python','scripts/video/transition_guard.py',m['output'],'--outdir',str(p/'transitions-splices')]
for x in m['boundaries']:
 if not x['label'].startswith('student-camera-'):cmd+=['--boundary',f"{x['frame']}:{x['label']}"]
