#!/usr/bin/env python3
"""Transformer source 1, with selected complete narration clauses from source 2. Review only."""
...
def main():
 ...
 keep(1,0,5.0,'Original Notebook opening')
 keep(1,13.5,26.6,'Tokens and starting embeddings')
 keep(1,26.6,36.6,'Context shapes meaning; safe original feather illustration','feather_intro',122.5)
 pause('Pause before language examples')
 keep(1,39.8,58.65,'LIGHT and pronouns','problems')
 pause('Pause before earlier AI')
 keep(1,74.75,82.3,'Original sequential-processing animation')
 keep(1,82.4,95.43,'How earlier AI carried CAT forward','before')
 pause('Pause before breakthrough')
 keep(1,98.0,108.7,'Original 2017 research and Transformer graphic')
 keep(1,108.78,116.43,'Complete message available from the start','reads')
 pause('Pause before relevance and number updates')
 keep(2,122.2,129.5,'Why relevance still needs to be calculated','feather_bridge',122.2)
 keep(1,151.3,162.6,'Relationships update the token numbers')
 pause('Pause before attention and transformation')
 keep(1,163.0,181.25,'Attention blends; transformation applies fixed patterns','operations')
 pause('Pause before returning examples')
 keep(2,170.3,171.95,'Return to the original examples','resolve')
 keep(2,177.55,181.96,'Turn on and carry provide context','resolve_left')
 keep(2,42.0,47.35,'Thirsty cat and fresh milk interpretations','resolve_right')
 pause('Pause before broader language applications')
 keep(2,204.1,215.35,'Relationships support sarcasm and idioms','gears',206.95)
 pause('Pause before word order')
 keep(1,222.9,246.5,'Different orders; positional information','order')
 ...
 def board_state(t,f):
  ...
  if key=='problems':
   if sec>=50.65:rect=[816,128,1560,1173];color=GREEN;label='problems-pronouns'
   elif sec>=44.25:rect=[40,128,784,1173];color=BLUE;label='problems-light'
   if rect:focus=None
  elif key=='before':
   ...
   if sec>=112.3:rect=[40,628,1560,716];label='reads-all-present'
  elif key=='operations':
   if sec>=176.78:rect=[816,128,1560,730];color=TEAL;label='operations-transformation'
 ...
 manifest['visual_entry_clamps']={'Original sequential-processing animation':fr(74.9),'Original 2017 research and Transformer graphic':fr(98.1)};manifest['native_video_seconds']=nativecount/30;manifest['render_sha256']=sha(DEST);manifest['protected_files_unchanged']={str(p):sha(p)==hashes[str(p)] for p in protected};assert all(manifest['protected_files_unchanged'].values())
