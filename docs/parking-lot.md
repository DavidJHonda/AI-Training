...
## "Trust, Then Check" — VerifySection wrapper prose (+ "Confidence is not consistency" KeyInsight)

- **Origin:** `VerifySection`, formerly embedded in Check the Results (`evaluating`). Removed 2026-07-03: after the six strategies collapsed to the static SIX CHECKS box, the wrapper was scaffolding from its days as a planned standalone Verify lesson — intro ("Is this true?… These six strategies will help you verify AI output"), a Step-4 recap paragraph duplicating the lesson's own handoff, "The strategies below are tools" filler, and the "Stack them, don't pick one" section that re-listed the checks in prose. The SIX CHECKS box and the "If you can't find the source, assume it doesn't exist" KeyInsight were inlined into `EvaluatingSection`; the component, its `VERIFY_STRATEGIES` const (whose desc/aiSays/doThis fields were already unrendered), and the vestigial `verify` SECTION_META entry were deleted.
- **Worth reclaiming:** the "Confidence is not consistency" KeyInsight — ask twice, don't average contradictory answers, the contradiction is the signal. A distinct idea with no current home; could fit Hallucination (Avoid the Traps).
- **Full source:** in git history at the commit before the 2026-07-03 "inline the six checks" commit — the whole `VerifySection` function; search anchor `Confidence is not consistency`.

---

## "Watch It Happen" — three-snapshot basketball SEE IT

- **Origin:** Training lesson (`TrainingSection`), removed 2026-07-09 when the three phase snapshots were folded into the phase boxes as per-phase "How it answers:" rows (with trimmed responses) plus a standalone Takeaway after Phase 3. The distributed rows keep the payoff local to each phase; this parked version is the single side-by-side comparison view.
- **Possible destination:** the Training lesson video (the same-question-three-answers comparison is a strong board candidate), or back into the lesson if the side-by-side contrast is ever wanted again.
- **Supporting state it needs if restored:** none live — it had already been converted to a static showcase (`visibleStages = seeItStages.length`, typing disabled). The `seeItQuestion`/`seeItStages`/`seeItBtnLabels` vars were removed in the same pass.
- **Verbatim stages** (question "How do I shoot a basketball?"):
  1. After Pretraining (the base model, #f59e0b): response "How do I shoot a basketball? The basketball shot is one of the most fundamental skills in the sport. In this guide, we will cover the basics of shooting form, including stance, grip, and follow-through..." — note "It can produce fluent text, but it doesn't know it's talking to a person. It just continues the sentence."
  2. After Instruction tuning (Supervised Fine-Tuning, #3b82f6): response "To shoot a basketball, square your feet to the hoop, put your shooting hand under the ball with your guide hand on the side, bend your knees, and push up, releasing off your fingertips with a follow-through." — note "Example conversations taught it to answer the person directly instead of just continuing the text. Helpful now, but still a little flat."
  3. After Preference tuning (RLHF, #10b981): response "Great question! The biggest thing beginners get wrong is using two hands to push the ball. Try this: elbow under the ball, flick your wrist like you're reaching into a cookie jar on a high shelf. Want tips on free throws vs three-pointers?" — note "Human rankings taught it to be engaging, use analogies, and offer follow-up. This is what you experience today."
- **Full source:** in git history at the commit before this one, inside `TrainingSection` — the ShowcaseBox headlined "Watch It Happen" (anchor string), with the "You" chat bubble, per-stage label/(phase)/note rows over white response cards, and the closing Takeaway "Fluent came first. Helpful was taught." (briefly kept after Phase 3, then cut 2026-07-09 — the per-phase How-it-answers rows demonstrate the insight; verbatim body: "By the base-model stage, the model could already produce fluent text about basketball. It took human examples and rankings to turn a sentence-completer into something that answers you: engaging, useful, and ready with a follow-up.").

---

## "Match the step to the phase" — TrainingPhaseMatch TRY IT
...
## "Which One Would You Reach For?" — REACH per-job app picker TRY IT

- **Origin:** Which App lesson (`ModelSelectionSection`), replaced 2026-08-03 by "It's in the Name" (name-origin quiz). Owner call: the lesson prescribes picking one home base, so a per-job app-choice drill rehearsed the behavior the lesson argues against — it also had no right answers (every pill passed `isCorrect: true` by design, with a comment noting that scoring one as right would contradict the lesson's own "any of the three works").
