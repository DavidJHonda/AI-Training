...
MEDIUM_32 = font("AvenirNextforINTUIT-Medium.otf", 32)
MEDIUM_34 = font("AvenirNextforINTUIT-Medium.otf", 34)
...
def header(draw, box, eyebrow, title, accent, kind):
    ...
    draw.text((x0 + 82, y0 + 7), eyebrow, font=HEAVY_28, fill=accent)
    draw.text((x0 + 82, y0 + 42), title, font=MEDIUM_34, fill=NAVY)
...
def comparison_card(draw, box, title, body, accent, background):
    ...
    draw.text((x0 + 24, y0 + 12), title, font=MEDIUM_32, fill=accent)
    draw_wrapped(draw, (x0 + 24, y0 + 58), body, MEDIUM_34, x1 - x0 - 48, gap=5)


def row_label(draw, box, lines):
    x0, y0, x1, y1 = box
    draw.rounded_rectangle(box, radius=16, fill=LABEL_BG, outline=LABEL_RULE, width=2)
    if len(lines) == 1:
        centered(draw, ((x0 + x1) / 2, (y0 + y1) / 2), lines[0], MEDIUM_28, NAVY)
    else:
        centered(draw, ((x0 + x1) / 2, (y0 + y1) / 2 - 16), lines[0], MEDIUM_28, NAVY)
        centered(draw, ((x0 + x1) / 2, (y0 + y1) / 2 + 16), lines[1], MEDIUM_28, NAVY)
...
def main():
    ...
    header(draw, (880, 192, 1488, 280), "AI SOFTWARE", "Built from patterns", PURPLE_DARK, "patterns")
    draw.ellipse((770, 208, 830, 268), fill=NAVY)
    centered(draw, (800, 238), "VS", MEDIUM_28, WHITE)
    ...
    for (y0, y1), left, right, label in rows:
        ...
        row_label(draw, (734, y0 + 38, 866, y1 - 38), label)
