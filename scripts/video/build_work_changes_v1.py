#!/usr/bin/env python3
"""Work Changes v1 review candidate from work-changes-4 (2026-09-24). Review only.
...
import cv2
from editspec_build import Build, fr, FPS, NEUTRAL, PURPLE, BLUE, TEAL, AMBER, banner_rect

ROOT = Path(__file__).resolve().parents[2]
P = ROOT / "Prompts"
SRC, R1, R2, R3 = (P / f"work-changes-{k}.mp4" for k in (4, 1, 2, 3))
A_ = ROOT / "course-assets/work-changes"
OUT = ROOT / "video-audit/work-changes-v1-2026-09-24/build"
DEST = P / "work-changes-v1.mp4"
...
A_QUOTE, A_BEFORE, A_WITH = [31, 85, 1170, 220], [31, 246, 587, 1272], [613, 246, 1170, 1272]          # 1203x1308
...
def main():
    ...
    b.keep(430, 680, "Notebook: notebook and tablet, Two Pillars diagram ('review two foundational concepts')")
    ...
    b.keep(3582, 3882, "Your First Assignment: Before AI, then you", "assignment")
    b.keep(3882, 4041, "BREAK: R3 Week 1 findings / Week 2 deep rework under 'The boss adds a page of comments...'",
           video_from=2835, video_src=R3, video_end=3021)
    b.keep(4041, 4491, "Your First Assignment: With AI", "assignment")
    b.keep(4491, 4650, "BREAK: R2 500 Raw Reviews -> AI Automation -> Structured Data under 'The AI reads the reviews, finds the themes...'",
           video_from=4575, video_src=R2, video_end=4674)
    b.keep(4650, 5096, "Your First Assignment: you start here, the result", "assignment")
    ...
    b.board("assignment", ASSIGN, 2811, 5096, "compact", [
        target("The assignment", 96.10, A_QUOTE, PURPLE),
        target("Before AI", 104.54, A_BEFORE, PURPLE),
        target("With AI", 135.68, A_WITH, AMBER),
    ], push=False)
    ...
    b.manifest({
        ...
        "board_breaks": [
            ...
            {"board": "assignment", "source_frames": [3882, 4041], "drawing": "R3 2835-3020 Week 1 / Week 2 rework"},
            {"board": "assignment", "source_frames": [4491, 4650], "drawing": "R2 4575-4673 500 Raw Reviews -> AI Automation -> Structured Data"}],
