...
## Lesson map
71 lessons across 10 section groups, listed in delivery order. Each lesson is listed as `Displayed Label (internal id)`. The id is the React component key and navigation reference used in code; the label is what students see in the UI.
...
- AI Answers (6): Opener (openeranswers), Your Input & the Context Window (prompt), Inference: From Prompt to Output (inference), Probability (probability), Prediction (prediction), Patterns (patterns)
- Workflow (9): Opener (openerusing), What AI Does Best (whatitdoesbest), Creative Thinking (creativethinking), Questions Matter (questionsvaluable), The Art of Prompting (prompting), Your Thought Partner (thoughtpartner), Critical Thinking (critical), How to Verify (verify), Evaluating the Results (evaluating)
