...
enough to need a camera move:
  ...
  B1 Same Probabilities, Different Choices  1570-3152  the probabilities panel, the Other row,
                                                       Five Random Picks (held through graft 1),
                                                       then the banner
...
PROBS, OTHER_ROW, PICKS = [88, 274, 659, 701], [96, 642, 651, 691], [948, 275, 1529, 730]   # the two headed panels ring outside their titles, not through them.
                                            # v8: the left panel is unchanged (rows 286-691); the right one now ends its five
                                            # rows at 625 and carries the new caption on three lines from 641 to 720, so its
                                            # ring runs to 730 and holds the whole column - heading, picks and caption.
...
def main():
    ...
    B1, B1_OUT = 1570, 3152        # in on Notebook's cut at 52.33; out on its cut at 105.07, where its recreation of
    TREE_IN, TREE_OUT = 3152, 3540 # our board ends and its branching-paths drawing begins (v9)
    B2, B2_OUT = 3540, 5013        # 118.00, in the silence before "So how do you control that variety?" (Notebook's
                                   # own cut at 118.20 is 2 frames into that line)
    ...
    b.keep(TREE_IN, TREE_OUT, "Notebook drawing: one different token, two different sentences (Max chases / Max sleeps)")
    ...
    b.board('1-draws', B['1-draws'], B1, B1_OUT + G1, 'compact',
            [T('The Probabilities', 57.28, PROBS, PURPLE),
             ...
             T('Five Random Picks', 74.40, PICKS, PURPLE)],
