...
- Start Smarter (8): Welcome (welcome), Why Learn AI? (whydeeper), What Is AI? (aihistory), What's an LLM? (llms), Does AI Think? (doesaithink), Why Bother Learning? (whybother), What You Can Control (control), The Roadmap (intro)
- Understand AI (16): one section group with a single opener; the former Inside AI and AI Answers sub-openers were removed and their framing folded into tokens (top) and training (end):
  - Foundations: Opener (openerfoundations), AI Primer (primer), AI is Math (aiismath), How We Got Here (howwegothere), Rules vs Patterns (aivscode), No One Wrote the Rules (norules), AI's Fuel (data)
  ...
  - How AI builds an answer: Your Input & the Context Window (prompt), Inference: From Prompt to Output (inference). The former Patterns lesson (patterns) was removed 2026-06-03 as redundant: its one unique idea (training writes patterns into the weights, inference reads them back) folded into the AI Primer, and the term "weights" now debuts in Training. The standalone Probability lesson (probability) was removed 2026-06-04 and folded into the new AI Primer (primer): the Primer teaches the patterns → training → probability → prediction loop in plain language; its SEE IT and TRY IT activities came from the old Probability lesson. Cut intro prose is logged in docs/parking-lot.md.
