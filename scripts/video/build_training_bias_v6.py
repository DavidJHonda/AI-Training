...
OUT = ROOT / "video-audit/training-bias-comparison-2026-09-21/build-v6"
DEST = ROOT / "Prompts/training-bias-v6.mp4"
# v6 (David, 2026-09-21): v5's card rects were measured against the stage colour and swallowed each card's drop
# shadow (4-8 px on the sides, 14 px below), so the rings floated outside the cards. These rects come from the
# illustration's own extent and the white area above the shadow. Everything else is identical to v4/v5.
...
def main():
    ...
    cards = [(41, 524), (558, 1042), (1076, 1559)]   # card edges = the illustration tile's x extent
    card = lambda i, top, bot: [cards[i][0], top, cards[i][1], bot]
    b.board("skew", SKEW, SKEW_IN, SKEW_OUT, "compact", [
        target("Defaults", 62.02, card(0, 128, 650), PURPLE),
        target("Blind Spots", 71.22, card(1, 128, 650), BLUE),
        target("Wrong Patterns", 78.20, card(2, 128, 650), AMBER),
    ], push=False)
    b.board("questions", QUESTIONS, Q_IN, Q_OUT, "compact", [
        target("Ask What's Missing", 120.70, card(0, 128, 650), PURPLE),
        target("Ask for Exceptions", 125.04, card(1, 128, 650), BLUE),
        target("Remove the Famous", 132.42, card(2, 128, 650), AMBER),
    ], banner_at=139.48, banner=[40, 691, 1560, 779], push=False)
    b.board("rag", RAG, RAG_IN, RAG_OUT, "compact", [
        target("Retrieve", 210.64, card(0, 128, 609), PURPLE),
        target("Add to Context", 216.98, card(1, 128, 609), BLUE),
        target("Generate", 224.92, card(2, 128, 609), TEAL),
    ], push=False)
