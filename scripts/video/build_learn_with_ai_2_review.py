...
def main():
    ...
    LB_END = B1_OUT + (R2_BANNER[1] - R2_BANNER[0])     # leg b: roll 1 frames XA2..B1_OUT, then the roll 2 banner line
    ...
    ta = [T(k, t, sec(c1[0], *SY[k]), BLUE) for k, t in (('header', 61.27), ('what', 63.50), ('best', r2a(43.40)), ('catch', r2a(50.84)))]   # best/catch at roll 2's onsets
    tb = [T(k, t, sec(c1[1], *SY[k]), PURPLE) for k, t in (('header', XA2 / 30), ('what', 92.06), ('best', 97.72), ('catch', 103.78))]      # header ringed from the leg's first frame (the Focus catch ring ends with leg a)
    b.board('1-study-tools', B['1-study-tools'], B1, LA_END, 'compact', ta, min_open=0, push=False)
    b.board('1-study-tools-b', B['1-study-tools'], XA2, LB_END, 'compact', tb, banner_at=r2b(90.90), min_open=0, push=False)
    ...
    b.manifest({'narration_cuts_source_frames': [[XA1, XA2], list(CUTA), list(CUTC)], 'replaced_garble_source_frames': list(GARBLE), 'graft_roll2_frames': list(GRAFT2), 'roll2_focus_frames': list(R2_FOCUS), 'roll2_banner_frames': list(R2_BANNER), 'cards_detected': {'1-study-tools': c1, '2-how-it-works': c2, '3-four-moves': c3}})
