...
c: "Once a word is chosen, it joins the text ... repeating the loop." after the 47% line on What Comes Next (v6: moved from after "far from certain." to the page's order; v5 never handed over).
...
OUT = ROOT / "video-audit/ai-is-math-comparison-2026-09-22/build-v6"
DEST = ROOT / "Prompts/ai-is-math-v6.mp4"
...
C_SPLIT = 5298     # 176.60: floor after "pie." (tail ends 176.26); graft c goes here in the page's order (after the 47% line); roll 1 resumes before "The question" 177.00 (v6, 2026-09-22)
...
def main():
    ...
    r1_hinge = speech_rms(OUT / "source.wav", [(115.74, 119.92)]); r1_certain = speech_rms(OUT / "source.wav", [(171.46, 176.26)])
    ...
    b.keep(B_IN, C_SPLIT, "What Comes Next: question, reply, chips, Spot, far from certain, remaining 47% (footnote ring)", "next")
    ...
    b.keep(C_SPLIT, P_SPLIT, "What Comes Next: banner line", "next", video_from=C_SPLIT + LC)
    ...
    b.board("next", NEXT, NEXT_IN, NEXT_OUT + LC, "compact", [
        ...
        target("footnote: Thousands of other possible words ... remaining 47%", 171.46, NEXT_FOOTNOTE, PURPLE),
    ...
    b.manifest({
        ...
        "narration_changes": {
            ...
            "speech_rms": {"roll1_hinge_115.74-119.92": r1_hinge, "roll2_donor_d_200.00-204.12": d_d, "roll1_47pct_line_171.46-176.26": r1_certain, "roll2_donor_c_258.78-267.62": d_c},
