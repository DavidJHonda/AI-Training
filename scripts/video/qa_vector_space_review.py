...
P=Path('video-audit/vector-space-repair-2026-09-10');m=json.loads((P/'edit-manifest.json').read_text());ff=imageio_ffmpeg.get_ffmpeg_exe()
...
for t in m['timeline']:
 ...
 if en-st>=.95:assert max(q[1]-q[0] for q in intervals)>=.95,(t['label'],intervals)
