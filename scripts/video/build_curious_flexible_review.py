#!/usr/bin/env python3
"""Build the review repair for the Curious & Flexible Notebook roll.

Six owner-approved narration spans are removed at measured room-tone
boundaries. Both current four-card lesson boards receive course-native,
outer-boundary walks, and the exact standard close replaces the Notebook
ending. The result is review-only and never overwrites a shipped video.
"""
...
SOURCE = ROOT / "Prompts/curious-_-flexible.mp4"
OUTPUT = ROOT / "Prompts/curious-and-flexible-patched.mp4"
TRANSITION_AUDIT = Path("/private/tmp/curious-flexible-transition-audit")
...
# Half-open frame spans. Every edge sits inside measured room tone, not on a
# Whisper word stamp. In particular, cut 3 ends before the acoustically late
# "But" even though Whisper places that word roughly 0.6 seconds early.
CUTS = (
    (1018, 1174),  # "survive our current technological landscape"
    (1521, 2178),  # invented adaptation-engine / radar framework
    (3276, 3404),  # "Once your radar ... curiosity's job is done"
    (3554, 4058),  # extended radar / phase-two / filter explanation
    (4905, 5117),  # "historical benchmark is the only accurate way"
    (5730, 5844),  # orphaned "final rule of the adaptation engine"
    (6249, 6533),  # two-part-engine anxiety summary
)
...
# Canonical outer card geometry, measured from the supplied board renders.
# Rings trace complete cards; they never use inset text or illustration boxes.
CURIOUS_BOUNDS = {
...
def teaching_legs() -> tuple[BoardLeg, BoardLeg]:
    curious = make_leg(
        ...
        BOARDS["curious"],
        (
            CUTS[1][1], at(76.66), at(84.22), at(91.98), at(99.94),
            CUTS[2][0],
        ),
        (
    ...
    # Five legacy radar frames sit immediately after the approved first-board
    # narration. Hold the final complete card across them, then resume on the
    # clean target illustration. Duration remains frame-identical.
    last = curious.states[-1]
    curious = BoardLeg(
        curious.name,
        curious.board,
        curious.source_start,
        curious.source_end + 5,
        curious.states[:-1] + (
            State(
                label=last.label,
                frames=last.frames + 5,
                ring=last.ring,
                color=last.color,
                camera=last.camera,
                move_frames=last.move_frames,
            ),
        ),
    )
    ...
    flexible = make_leg(
        ...
        BOARDS["flexible"],
        (
            CUTS[3][1], at(142.74), at(156.20), CUTS[4][0], CUTS[5][0],
            CUTS[6][0],
        ),
        (
            ("full", None, VIDEO_PURPLE, None, 0),
...
def main() -> None:
    ...
    close_frames = expected - output_frame(CUTS[6][0])

    with tempfile.TemporaryDirectory(
        prefix="curious-flexible-review-", dir="/private/tmp"
    ) as directory:
        work = Path(directory)
        ...
        append_source_video(graph, video_labels, CUTS[0][1], CUTS[1][0])
        append_leg_video(graph, video_labels, 1, curious.frames)
        append_source_video(graph, video_labels, CUTS[2][1] + 5, CUTS[3][0])
        append_leg_video(graph, video_labels, 2, flexible.frames)
    ...
    boundaries = (
        (output_frame(CUTS[0][0]), "cut-landscape"),
        (output_frame(CUTS[1][0]), "source-to-curious"),
        (
            output_frame(CUTS[1][0]) + curious.frames,
            "curious-to-source",
        ),
        (output_frame(CUTS[3][0]), "source-to-flexible"),
        (output_frame(CUTS[6][0]), "flexible-to-close"),
    )
    command = [
        sys.executable,
