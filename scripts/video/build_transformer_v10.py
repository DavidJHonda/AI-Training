...
OUT = ROOT / "video-audit/transformer-comparison-2026-09-22/build-v10"
DEST = ROOT / "Prompts/transformer-v10.mp4"
...
LIVE_2017_IN, LIVE_2017_OUT = 3305, 3628   # live v8 frames under the graft (v10)
...
B5_DM_CLUE, B5_PR_CLUE = [74, 994, 750, 1212], [850, 994, 1526, 1212]   # v10: the tinted clue boxes themselves (measured 74-750 / 850-1526 x 994-1212, corner radius ~10); the label above them is not ringed   # "WHICH WORDS PROVIDE THE CLUES?" label (963-976) + box 994-1212; 16 px above the card bottom
...
def main():
    ...
    # v10 (David 2026-09-22): the live v8's own drawing for this beat sits under the grafted sentences instead of the held ChatGPT-T frame:
    # live 3305-3628 (1:50.17-2:00.93; its cuts at 110.17 and 120.93), the 2017 / Attention Is All You Need card dissolving to the Transformer / ChatGPT breakdown.
    b.rows[-1]["video_src"] = str(LIVE); b.rows[-1]["video_start"] = LIVE_2017_IN; b.rows[-1]["video_end"] = LIVE_2017_OUT
    assert LIVE_2017_OUT - LIVE_2017_IN >= DONOR[1] - DONOR[0], "live span shorter than the graft"
    ...
    b.finish_audio()
    # v10 (David 2026-09-22: "strange sounds after the closing message"): the framework's room tone is a 0.1 s seed mirrored into a
    # 0.2 s loop; on this roll the seed has a level ramp, so the 4 s close hold pulsed at 5 Hz (autocorrelation 1.00 at 0.200 s,
    # RMS swinging -49 to -54 dBFS). Keep 0.4 s of tone after the last word's tail, then fade it to silence over 1.2 s.
    import numpy as _np
    from editspec_build import readwav as _rd, writewav as _wr, SR as _SR, SPF as _SPF
    _ed = _rd(b.out / "edited.wav"); _hold = next(r for r in b.rows if r["label"] == "Settled close hold")
    _s0 = _hold["start_frame"] * _SPF + int(0.4 * _SR); _n = int(1.2 * _SR)
    _ed[_s0:_s0 + _n] *= _np.linspace(1.0, 0.0, _n); _ed[_s0 + _n:] = 0.0
    _wr(b.out / "edited.wav", _ed)
    ...
    b.board("earlier", B2, CUT_IN, GRAFT_OUT, "compact", [
        ...
        target("unmarked from By the time the AI reaches the word it (v10: the board itself carries an 8 px purple border on IT; a ring on top doubled it)", 73.92, None, PURPLE),
    ...
    b.board("resolves", B5, B5_IN, B5_OUT, "compact", [
        target("Problem 1 clue block: turn on tells the AI light means brightness", 147.94, B5_DM_CLUE, BLUE, radius=12),
        target("Problem 2 clue block: Thirsty describes the cat", 156.76, B5_PR_CLUE, GREEN, radius=12),
    ...
    b.manifest({
        "v10_changes": "IT chip ring removed (board already borders IT); clue rings hug the tinted boxes; live v8 3305-3628 under the graft; close hold tone faded to silence (0.4 s hold + 1.2 s fade) to remove the 5 Hz loop pulse.",
