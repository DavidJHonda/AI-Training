...
## Course structure
10 section groups, 59 lessons total. Each lesson is listed as `Displayed Label (internal id)`. The id is the React component key and navigation reference used in code; the label is what students see in the UI.
...
- Controls (6): Choosing the Product (modelselection), Choosing the Model (choosemodel), Thinking Mode (thinkingmode), Temperature (temperature), Customization & Memory (customization), Document Chat (documentchat)
- AI Traps (6): Hallucination (hallucination), The Mind Trap (mindtrap), The Flattery Trap (flattery), The Engagement Trap (engagementtrap), The Support Trap (supporttrap), When AI Acts (whenaiacts)
- Human Judgment (4): The AI Trust Test (aistrengths), Critical Thinking & AI (critical), Questions Matter (questionsvaluable), Skills That Matter (humanedge)
- Using AI (5): The Art of Prompting (prompting), Your Thought Partner (thoughtpartner), Studying With AI (studying), How to Verify (verify), Evaluating the Results (evaluating)
- AI in the Real World (6): Academic Integrity (integrity), Privacy & Awareness (privacy), When AI Judges You (judged), Seeing Isn't Proof (synthetic), Work Changes (workchanges), AI & The Future (aifuture)
...
- Planned: five remaining section opener pages — openercontrols, openertraps, openerjudgment, openerusing, openerrealworld. Three openers built so far: openerfoundations, openerinside, openeranswers. The OpenerSection helper component (see Consistency push) handles the format; each new opener is a thin wrapper that passes content props. Content not yet drafted for the remaining five. Placeholder pages are explicitly not added to index.html until each opener has real content, since validators require every SECTIONS entry to have a SECTION_COMPONENTS mapping.
