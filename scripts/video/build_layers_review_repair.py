...
OUT=ROOT/'video-audit/layers-repair-2026-09-10';DEST=ROOT/'Prompts/layers-v2.mp4'
...
def main():
 ...
 protected=[*src.values(),ROOT/'course-assets/layers/layers.mp4',ROOT/'index.html',ROOT/'lessons/layers.md']
 ...
 keep(1,85.5,91.266667,'Complete CAT and IT sentence')
 donor_start=cursor
 keep(2,77.5,83.4,'Accurate START explanation from version 2',91.3)
 keep(1,96.133333,112.8,'Layers connect IT to CAT; zero-context claim removed')
 pause('Pause before layer count',112.766667)
 keep(1,113.1,145.433333,'Layer counts and depth for nuance')
 pause('Pause before computing tradeoff',145.4)
 keep(1,145.633333,154.533333,'Computing power and time tradeoff')
 ...
 ev(22.16,'horse','horse-first-read',[80,155,538,782],'#4f2fc4')
 ev(24.58,'horse','horse-more-reads',[571,155,1029,782],'#1652f0')
 ev(30.3,'horse','horse-meaning-clicks',[1062,155,1520,782],'#0e8f86')
 ...
 ev(78.466667,'reader','notebook-reader-meaning')
 ...
 ev(101.04,'it','it-result',[1249,330,1521,754],'#4f2fc4')
 ...
 def source_at(f):
  ...
  return t.get('visual_hold_frame',t['source_start']+f-t['start_frame'])
 ...
 manifest=dict(source=str(src[1]),donor=str(src[2]),output=str(DEST),fps=30,total_frames=total,duration=total/30,close_start_frame=close_start,timeline=timeline,states=schedule,protected_hashes=hashes,outline_checks=checks,board_assets={k:str(p) for k,p in paths.items()},boundaries=[dict(frame=f,label=l) for f,l in sorted(boundaries.items())],audio=dict(room_tone_source=[112.68,112.86],crossfade_ms=10,sample_rate=SR,donor_gain=gain,donor_output_start=donor_start/30),close_camera=dict(prehold_frames=48,push_frames=150,zoom_endpoint=1.2,settled_frames=total-close_start-198),visual_replacements={'stack':'Incorrect separate-stage diagram replaced only during layer explanation','book':'Version 2 original books replace false IT/CAT vector-matching diagram','reader':'Version 1 rereading illustration replaces unrelated cat/ice/ruler bridge'})
 ...
 readers={'native':Reader(src[1]),'book':Reader(src[2]),'reader':Reader(src[1])}
 def clean(frame):
  # Remove only the generated corner wordmark; no course or stock asset is cropped.
  f=frame.copy();roi=f[691:712,1145:1276];gray=cv2.cvtColor(roi,cv2.COLOR_BGR2GRAY)
  mask=((gray>150)&(gray<246)).astype(np.uint8)*255;mask=cv2.dilate(mask,np.ones((2,2),np.uint8))
 ...
 for f in range(total):
  if f<close_start:
   ...
   if key in readers:
    ...
    elif key=='reader':sf=min(fr(38.566667)+f-e['start_frame'],fr(43.266667))
    else:sf=source_at(f)
