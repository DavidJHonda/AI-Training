...
7. Publish the website changes together when ready, including `index.html`, `country-select.js`, and `course-assets/the-final/the-final-certificate-sample.jpg` if not already published. The backend also accepts the earlier country form, so it can be updated before the website.
