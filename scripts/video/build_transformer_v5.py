#!/usr/bin/env python3
"""Owner revisions to full Transformer video: board arrows, pause and audio cleanup. Review only."""
...
ROOT=Path(__file__).resolve().parents[2];OUT=ROOT/'video-audit/transformer-full-2026-09-10/v5';DEST=ROOT/'Prompts/transformer-v5.mp4';ASSETS=ROOT/'video-audit/transformer-repair-2026-09-10'
...
def main():
 ...
 protected=[*sources.values(),ROOT/'Prompts/transformer-v2.mp4',ROOT/'Prompts/transformer-v3.mp4',ROOT/'Prompts/transformer-v4.mp4',ROOT/'course-assets/transformer/transformer.mp4',ROOT/'index.html',ROOT/'lessons/transformer.md']
 ...
 keep(1,131.0,140.6,'Full explanation of calculating relevance','relevance')
 ...
 pause('Pause between LIGHT and CAT examples')
 keep(2,42.0,47.35,'Cat and milk interpretations','resolve_right')
 keep(1,199.6,206.3666666667,'Original complex-phrasing explanation')
 pause('Pause before word order')
 keep(1,223.1,246.5,'Full word-order comparison and positional encoding','order')
 ...
 paths={k:ROOT/('course-assets/transformer/transformer-'+s+'-editorial.jpg') for k,s in {'problems':'context-problems','before':'before-transformers','reads':'how-transformer-reads','operations':'attention-transformation','resolve':'resolves-meaning','order':'word-order'}.items()};paths['close']=ASSETS/'close.png'
 ...
 def board_state(t,f):
  ...
  if key=='problems':
   ...
   key='reads';rect=[696,450,805,538];color=NEUTRAL;label='reads-calculate-relevance'
  elif key=='reads':
 ...
 # Owner-requested attention paths, kept in the board's whitespace.
 # All three paths look backward from IT; CAT is emphasized without invented scores.
 def attention_paths(im,e,elapsed):
  out=im.copy();s,x,y=camera(e)
  def bezier(pts):
   q=np.linspace(0,1,100)[:,None];a,b,c,d=np.array(pts,float)
   return (1-q)**3*a+3*(1-q)**2*q*b+3*(1-q)*q*q*c+q**3*d
  paths=[
   dict(name='mat',delay=.7,duration=1.3,color=(205,190,176),width=2,segments=[[(784,446),(875,436),(1190,449),(1216,429)],[(1216,429),(1260,403),(1238,286),(1208,287)]]),
  ...
  return out
 relevance_state=next(e for e in schedule if e['label']=='reads-calculate-relevance')
 for elapsed in [0,2,4,7,9]:
  cv2.imwrite(str(OUT/'states'/f'relevance-{elapsed}.png'),attention_paths(cache[relevance_state['label']],relevance_state,elapsed))
 ...
 for f in range(total):
  ...
  if f>=close_start:
   ...
   if key in readers:
    ...
    if t['label']=='Original complex-phrasing explanation':idx=max(idx,fr(199.8))
    ...
    if si>0 and schedule[si-1]['board']==e['board'] and f-e['start_frame']<18:
     ...
     im=render(e,cam=cam)
  if t.get('visual')=='relevance':im=attention_paths(im,e,(f-t['start_frame'])/30)
 ...
 manifest['visual_entry_clamps']={'Human understanding, mathematical work, and earlier AI':fr(58.9),'Original 2017 research and Transformer graphic':fr(98.1),'Recall exercise and why the numbers must change':fr(140.7333333333),'Original complex-phrasing explanation':fr(199.8)};manifest['approved_word_cuts']=[dict(word='instantly',source_seconds=[120.5333333333,121.2]),dict(word='physical',source_seconds=[124.7666666667,125.1666666667])];manifest['owner_requested_arrow_overlay']=dict(span_original_v4=[137.5,147.1],paths=['IT to MAT','IT to RAINSTORM','IT to CAT'],source_board_unchanged=True);manifest['owner_narration_cut_source_seconds']=[206.3666666667,211.5333333333];manifest['order_entry_cleanup_source_seconds']=[222.7,223.1];manifest['native_video_seconds']=nativecount/30;manifest['render_sha256']=sha(DEST);manifest['protected_files_unchanged']={str(p):sha(p)==hashes[str(p)] for p in protected};assert all(manifest['protected_files_unchanged'].values())
