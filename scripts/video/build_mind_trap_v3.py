#!/usr/bin/env python3
"""Build Mind Trap v3 from mind-trap-6 (2026-09-21 seven-way review: rolls 1-6 and the live video).
...
OUT = ROOT / "video-audit/mind-trap-comparison-2026-09-21/build-v3"
DEST = ROOT / "Prompts/mind-trap-v3.mp4"
...
DEF_IN, DEF_OUT = 2296, 2450     # v2: both boundaries pulled back so the 5 ms row crossfade lands in silence, not on "This illusion is not new." (81.791) 
PHOTO_IN, PHOTO_OUT = 2450, 3160 # the archival photograph, replaced by the live videos ELIZA teletype; PHOTO_OUT stays at roll 6 own cut (0.03 s before 105.357)
ELIZA_IN, ELIZA_OUT = 3370, 5008 # v2: 1:52.33, 0.14 s before "Today's chatbots" (112.471); held 0.27 s past "human." (166.66) into the pause
GRAFT_IN, GRAFT_OUT = 4434, 4762 # roll 5: 147.80 (its own speech ends 147.75) to 158.73 ("lived alongside you." ends 158.64)
TAIL_IN, TAIL_OUT = 5008, 5020   # the rest of roll 6's post-takeaway pause, under roll 5's picture; the audio boundary sits 0.085 s before "Taking a chatbot's advice" (167.418)
RESUME_PICTURE = 5023            # v3: roll 6 holds Notebook's own board render through 5022, so the resumed picture starts at roll 6's own cut and runs 3 frames (0.1 s) ahead of its audio
...
def main():
    ...
    b.keep(TAIL_OUT, CLOSE_IN, "Notebook drawings: stakes slider, the middle move, prepare vs decide", video_from=RESUME_PICTURE)
    b.mark_close_start()
    ...
    b.manifest({
        ...
        "picture_advance": {"row_start_source_frame": TAIL_OUT, "picture_start_source_frame": RESUME_PICTURE,
                            "reason": "roll 6 holds Notebook's own ELIZA board render to frame 5022 while its next sentence starts at 167.418, so the audio boundary is placed in the pause and the resumed picture starts at roll 6's own cut; frames 5020-5022 of the roll are dropped and the picture leads its audio by 0.1 s for the rest of the body"},
