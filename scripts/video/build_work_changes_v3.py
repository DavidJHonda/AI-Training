#!/usr/bin/env python3
"""Work Changes v3 review candidate from work-changes-4 (2026-09-25). Review only.

v3 (David, 2026-09-25): the lesson's Board 4 was rewritten ("How Work Is Changing"; no study statistic). v3 keeps v2
and replaces roll 4 241.0-265.0 (the old board's intro and explanation, including v2's roll 1 study graft) with
roll 5 176.2-214.4 ("When you put automation and augmentation together ... that require your direct judgment."),
which David chose. Silences: r4 240.95-241.05 / 264.35-265.10, r5 175.45-176.70 / 214.00-214.80. Roll 5 speech
measures -20.1 LUFS against roll 4's -19.4 / -18.7 either side: +1.0 dB. Roll 5 draws only its own board render
here, so the canonical board is broken twice with roll 1 drawings (office floors; glasses and pen).
...
P = ROOT / "Prompts"
SRC, R1, R2, R3, R5 = (P / f"work-changes-{k}.mp4" for k in (4, 1, 2, 3, 5))
A_ = ROOT / "course-assets/work-changes"
OUT = ROOT / "video-audit/work-changes-v3-2026-09-25/build"
DEST = P / "work-changes-v3.mp4"
...
C_CARDS = [[41, 128, 525, 773], [558, 128, 1043, 773], [1076, 128, 1560, 773]]                         # 1600x814 (How Work Is Changing, 2026-09-25)
R5_IN, R5_OUT, R5_GAIN = 5286, 6432, 1.0   # roll 5 176.2-214.4


def leg_t(t5):
    """Roll 5 seconds -> board leg seconds (the leg is laid on roll 4 frames from 7230)."""
    return 7230 / FPS + (t5 - R5_IN / FPS)
...
def main():
    ...
    args = ap.parse_args()
    b = Build(ROOT, SRC, OUT, DEST, protected=[R1, R2, R3, R5, STRENGTHS, ASSIGN, TWOWAYS, CHANGES, CLOSE, LESSON, ROOT / "index.html"])
    ...
    # --- board 4
    # roll 4 7230-7950 (old board intro and explanation) replaced by roll 5 5286-6432, in four rows for the pictures
    R1b = OUT / "donor-r1b.mp4"   # a second roll 1 reader: the office drawing precedes frames already read
    if not R1b.exists(): os.symlink(R1, R1b)
    b.graft(R5, R5_IN, 5652, "GRAFT roll 5 under How Work Is Changing (canonical): 'When you put automation and augmentation together ... More Kinds of Work ... With AI's help, you can confidently handle tasks'",
            "hw1", picture_from=7230, visual="changes", gain_db=R5_GAIN)
    b.graft(R5, 5652, 5769, "GRAFT roll 5 / BREAK: R1 office floors under 'outside your traditional specialty, such as design, marketing, or customer support.'",
            "hw2", picture_from=7230, video_end=7231, gain_db=R5_GAIN)
    b.rows[-1].update(video_src=str(R1b), video_start=135, video_end=264)
    b.graft(R5, 5769, 6237, "GRAFT roll 5 under How Work Is Changing: More Productive, More Meaningful ('With routine tasks out of the way,')",
            "hw3", picture_from=7230 + 5769 - R5_IN, visual="changes", gain_db=R5_GAIN)
    b.graft(R5, 6237, R5_OUT, "GRAFT roll 5 / BREAK: R1 glasses and pen under 'you have the time to solve complex problems, develop ideas, and make decisions that require your direct judgment.'",
            "hw4", picture_from=7230, video_end=7231, gain_db=R5_GAIN)
    b.rows[-1].update(video_src=str(R1), video_start=4047, video_end=4254)
    ...
    for prev, row in zip(b.rows, b.rows[1:]):
        ...
        elif (prev.get("graft_audio") == row.get("graft_audio") == str(R5) and prev["audio_end"] == row["audio_start"]):
            r5 = readwav(OUT / "graft-hw1.wav") * 10 ** (R5_GAIN / 20)
            o, so = row["start_frame"] * SPF, row["audio_start"] * SPF
            edited[o - 240:o + 240] = r5[so - 240:so + 240]; restored.append(row["start_frame"])
    ...
    b.board("changes", CHANGES, 7230, 7230 + R5_OUT - R5_IN, "compact", [   # rings at roll 5's spoken onsets
        target("More Kinds of Work", leg_t(183.62), C_CARDS[0], PURPLE),
        target("More Productive", leg_t(193.26), C_CARDS[1], BLUE),
        target("More Meaningful", leg_t(203.62), C_CARDS[2], TEAL),
    ], push=False)
    ...
    b.manifest({
        ...
        "narration_changes": {
            "cuts": [{"source_frames": [a, c], "source_seconds": [s(a), s(c)], "text": t} for a, c, t in [
                ...
                (7230, 7950, "Roll 4's explanation of the old What Changes with AI board, 'This graphic shows three career shifts ... decide, and recommend.' (replaced by roll 5)"),
            ...
            "grafts": [
                ...
                {"source": "Prompts/work-changes-5.mp4", "source_frames": [5286, 6432], "gain_db": 1.0, "text": "When you put automation and augmentation together, three overarching changes appear across almost every career path. First, you take on more kinds of work. With AI's help, you can confidently handle tasks outside your traditional specialty, such as design, marketing, or customer support. Second, you become more productive. AI helps you complete routine tasks faster, allowing you to accomplish a higher overall output in the exact same timeframe. Third, your schedule becomes more meaningful. With routine tasks out of the way, you have the time to solve complex problems, develop ideas, and make decisions that require your direct judgment."},
        ...
        "board_breaks": [
            ...
            {"board": "changes", "roll5_frames": [5652, 5769], "drawing": "R1 135-263 office floors with different workstations"},
            {"board": "changes", "roll5_frames": [6237, 6432], "drawing": "R1 4047-4253 glasses and pen on papers"}],
