...
P=Path('video-audit/transformer-full-2026-09-10/v6')
...
pauses=[]
for lo,hi,label in [(204.9666667,205.9666667,'Preserved LIGHT to CAT pause'),(218.0666667,219.0666667,'Preserved pause before word order'),(238.6666667,239.0666667,'Short natural sentence gap'),(256.4,257.4,'Pause before close')]:
...
with wave.open(str(P/'final-position-narration.wav'),'wb') as w:
    ...
    w.writeframes(b[round(217*48000):round(259*48000)].astype(np.int16).tobytes())
...
assert all(Path(p).name=='index.html' and m['protected_files_unchanged'][p] for p in concurrent_changes),concurrent_changes
