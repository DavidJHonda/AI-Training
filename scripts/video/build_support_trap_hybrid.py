...
SOURCES=[ROOT/'Prompts/support-trap.mp4',ROOT/'course-assets/support-trap/support-trap.mp4']
P,B,A,T,R='#6e51ff','#1652f0','#a9760c','#0e8f86','#c41f28'
BOARDS={'compare':ROOT/'course-assets/support-trap/support-trap-comparison-v2.jpg','role':ROOT/'course-assets/support-trap/support-trap-role.jpg','danger':ROOT/'course-assets/support-trap/support-trap-danger.jpg'}
states=[];chunks=[]
...
def main():
    ...
    protected={str(p):sha(p) for p in SOURCES+list(BOARDS.values())+[ROOT/'course-assets/support-trap/support-trap-close.jpg']}
    canvases={k:v.build_canvas(p) for k,p in BOARDS.items()}
    close=cv2.resize(cv2.imread(str(ROOT/'course-assets/support-trap/support-trap-close.jpg')),(1600,900),interpolation=cv2.INTER_AREA)
