...
- SEE IT Pattern A spec established with Verify "Verification Strategies" as reference implementation. CSS vars --seeBand, --seeAccent, --seeRule added to :root. InteractiveBox and RevealSequence both gained a "sand" surface case alongside the existing "mint" case. Pattern B spec deferred until first parallel-reveal candidate is migrated. SEE IT migration scope: roughly 19 remaining SEE IT activities split across Pattern A (most are progressive), Pattern B (3-4 comparison-based), and SEE IT bespoke (Tokenize It, typing demos, Meaning Map, Watch It Happen, "Three Steps to an Answer" with its multi-step typing+pills+probability bars interior — these inherit the sand band and amber chrome but keep unique interiors). The Pattern A migration after this reference can proceed as small per-activity prompts that reuse the Verify shape with different data; Pattern B and bespoke each need their own design conversation. Future waves to extract TipCard (icon + title + desc pattern), StageCard (colored-border ladder pattern, used in 3+ ShowcaseBoxes), and HighlightBox (dark gradient highlight pattern) once enough usages are catalogued. Decision pending whether to demote casual 💡 lightbulb tips to plain bold body paragraphs (current lean) or formalize as a separate small Tip pattern. Smaller standardization passes still pending: h3 spacing, info-callout format consistency, two-column compare blocks. Wave 6 (Bucket D Pattern 2 interior migration for HumanEdge and WorkChanges) and bespoke-chrome normalization across the 13 legitimately bespoke TRY IT activities both deferred until after twin user testing. Twin testing identified as the next priority over further consistency work, with the SEE IT Pattern A reference being a tactical exception because the chrome work was needed for visual coherence regardless of testing.
