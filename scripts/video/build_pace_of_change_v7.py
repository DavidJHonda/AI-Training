#!/usr/bin/env python3
"""Pace of Change v7 review candidate from pace-of-change-4 (2026-09-24 roll). Review only.
...
OUT = ROOT / "video-audit/pace-of-change-rerolls-2026-09-24/build-v7"
DEST = P / "pace-of-change-v7.mp4"
...
CARDS_END = 525             # 17.50: last clean frame of the four capability cards before the EXPONENTIAL TRAJECTORY morph
...
CIRCUIT_AT = 4447           # 148.23 (147.88-148.59): "This diagram outlines what exactly is driving this relentless acceleration."
B2_IN = 4600                # 153.33 (153.06-153.42): "There are three mechanical concepts..."
...
R4_CIRCUIT = (4287, 4457)   # circuit sketch, torn-paper reveal
...
def main():
    ...
    b.keep(PHONE_AT, CIRCUIT_AT, "roll 2's phone -> check, then check/X over roll 4's GEN N diagram: Because of this constant replacement ... today's no is not necessarily permanent.", video_from=R2_PHONE_CHECK[0], video_src=R2, video_end=R2_PHONE_CHECK[1])
    b.keep(CIRCUIT_AT, B2_IN, "roll 4's circuit sketch: This diagram outlines what exactly is driving this relentless acceleration.", video_from=R4_CIRCUIT[0], video_end=R4_CIRCUIT[1])
    b.keep(B2_IN, DC_BREAK[0], "Why So Fast? (canonical): three mechanical concepts ... better training ... The second driver is more compute.", "fast")
    ...
    b.keep(HAND_BREAK[1], CUT4[0], "Why So Fast? (full view): Slow down and consider that third concept again. AI is already helping people build better AI.", "fast")
    ...
    b.manifest({
        "scope_detail": "v7: roll 4 as the base (David 2026-09-24, 'Build it'): four narration cuts, roll 4's own animated Board 1 after ~7 s of the canonical board, canonical Boards 2-4, race beat and Board 2 breaks from rolls 1-4, roll 4's forked spectrum under the not-a-timeline line, standard close.",
        ...
        "covered_spans": [
            ...
            {"frames": [MOM_AT, CIRCUIT_AT], "what": "ENGINE OF ACCELERATION, release cadence, ChatGPT/Claude/Gemini cards with invented token counts, GEN N diagram", "cover": "roll 3 MOMENTUM, roll 2 phone and check/X"},
