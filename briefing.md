...
- Inside the Model (8): Opener (openerinside), Tokens (tokens), Embeddings: Meaning as Numbers (embeddings), Vector Space: Meaning by Similarity (vectorspace), Behind the Numbers (behindthenumbers), Transformer Architecture (howreads), Training (training), Training Bias (trainingbias)
- How AI Answers (8): Opener (openeranswers), Context Window (context), Probability (probability), Prediction (prediction), Layers (layers), Attention (attention), Update the Meaning (transform), From Prompt to Output (inference)
...
- AI Traps (7): Opener (openertraps), Hallucination (hallucination), The Mind Trap (mindtrap), The Flattery Trap (flattery), The Engagement Trap (engagementtrap), The Support Trap (supporttrap), When AI Acts (whenaiacts)
...
- AI in the Real World (7): Opener (openerrealworld), Academic Integrity (integrity), Privacy & Awareness (privacy), When AI Judges You (judged), Seeing Isn't Proof (synthetic), Work Changes (workchanges), AI & The Future (aifuture)
- Finish Line (5): What You Learned (whatyoulearned), Full Workflow (fullworkflow), Vocab Quiz (keyterms), Test Yourself (testyourself), Beat the Clock (headtohead)
